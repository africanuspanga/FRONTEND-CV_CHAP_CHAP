...
/**
 * Create new anonymous CV
 */
export async function createAnonymousCV(req: Request, res: Response) {
  try {
    ...
    // Direct DB insert to bypass the foreign key constraint
    const cv = await storage.createRawCV({
      id,
      templateId,
    ...
    console.error('Error creating anonymous CV:', error);
    res.status(500).json({ message: 'Failed to create anonymous CV' });
  }
}
