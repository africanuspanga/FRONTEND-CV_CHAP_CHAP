...
async function uploadTemplate(templateId, templateName, htmlContent, description = '') {
  ...
  if (!response.ok) {
    throw new Error(`Failed to upload template: ${result.error || response.statusText}`);
  }
  
  console.log(`Template "${templateName}" (${templateId}) uploaded successfully`);
  return result;
}
...
// Function to upload all templates
async function uploadAllTemplates() {
  ...
  for (const template of templates) {
    try {
      await uploadTemplate(template.id, template.name, template.html, template.description);
      successCount++;
    } catch (error) {
      failCount++;
      console.error(`Error uploading template ${template.id}:`, error);
    }
  }
  
  console.log(`Upload complete. ${successCount} templates uploaded successfully, ${failCount} failed.`);
}
